...
def fast_feat_array(sequence, channel):
    ...
    mode_value1 = stats.mode(np.round(sequence, decimals=1), axis=None, keepdims=True)
    mode_value2 = stats.mode(np.round(sequence, decimals=2), axis=None, keepdims=True)
    mode_value3 = stats.mode(np.round(sequence, decimals=3), axis=None, keepdims=True)
    fdf = freq_dom_feat(sequence)
